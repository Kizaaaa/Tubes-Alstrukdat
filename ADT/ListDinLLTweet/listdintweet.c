...
void printListDinT(ListDinT l)
{
    ...
    else
    {
        IdxType i;
        printf("[");
        for (i = getFirstIdxListDinT(l); i < NEFF(l) - 1; i++)
        {
            printf("%d,", ELMT(l, i));
        }
...
ListDinT plusMinusList(ListDinT l1, ListDinT l2, boolean plus)
{
    IdxType i;
    ListDinT ln;
    CreateListDinT(&ln, CAPACITY(l1));
    NEFF(ln) = NEFF(l1);
    if (plus)
    { // dijumlahkan
        for (i = getFirstIdxListDinT(l1); i < NEFF(l1); i++)
        {
            ELMT(ln, i) = ELMT(l1, i) + ELMT(l2, i);
        }
    }
    else
    { // dikurangkan
...
IdxType indexListDinTOf(ListDinT l, ElType val)
{
    IdxType ans, i;
    ans = IDX_UNDEF;
    for (i = getFirstIdxListDinT(l); i < NEFF(l); i++)
    {
        if (ELMT(l, i) == val)
        {
            ans = i;
...
{
    *max = ELMT(l, getFirstIdxListDinT(l));
    *min = ELMT(l, getFirstIdxListDinT(l));
    IdxType i;
    for (i = getFirstIdxListDinT(l); i < NEFF(l); i++)
    {
        if (*max < ELMT(l, i))
...
{
    CreateListDinT(lOut, NEFF(lIn));
    NEFF(*lOut) = NEFF(lIn);
    IdxType i;
    for (i = getFirstIdxListDinT(lIn); i < NEFF(lIn); i++)
    {
    ...
    if (isEmptyListDinT(*l) == false)
    {
        if (asc)
        {
            ...
            while (bool)
            {
                ...
                if (ELMT(*l, NEFF(*l) - 2) > ELMT(*l, NEFF(*l) - 1))
                {
                    int temp = ELMT(*l, i);
                    ELMT(*l, i) = ELMT(*l, i + 1);
                    ELMT(*l, i + 1) = temp;
...
{
    ELMT(*l, getLastIdxListDinT(*l) + 1) = val;
    NEFF(*l)
    ++;
}
...
{
    *val = ELMT(*l, getLastIdxListDinT(*l));
    NEFF(*l)
    --;
    // printList(*l);
}
...
{
    int *ptr;
    ptr = (ElType *)calloc(CAPACITY(*l), sizeof(ElType));
    int *temp = ptr;
    CAPACITY(*l) = CAPACITY(*l) + num;
    ptr = realloc(ptr, CAPACITY(*l) * sizeof(ElType));
}
