...
void MuatKicauan(char *Path, ListDinT *KicauGlobal)
{
    ...
    *folder = '\0';
    STARTFILEWORD(path);
    NEFFT(*KicauGlobal) = EntryToInt(currentEntry);
    ADVLINE();
    long long int n = NEFFT(*KicauGlobal);
    ...
    {
        Kicau k;
        ID(k) = EntryToInt(currentEntry);
        ADVLINE();
        TEXT(k) = currentEntry;
        ADVLINE();
        TAGAR(k) = currentEntry;
        ADVLINE();
        LIKE(k) = EntryToInt(currentEntry);
        ADVLINE();
        AUTHOR(k) = currentEntry;
        ADVLINE();
        WAKTU(k) = entryToDateTime(currentEntry);
        ADVLINE();
        UTASAN(k) = NULL;
        BALASAN(k) = NULL;
        IDBALASAN(k) = 0;
...
void MuatBalasan(char *Path, ListDinT *KicauGlobal)
{
    ...
    *folder = '\0';
    ...
    while (n--)
    {
        ...
        IDKicauan = EntryToInt(currentEntry);
        ADVLINE();
        // printf("Memiliki "); printEntry(currentEntry); printf(" Balasan\n");
        t = EntryToInt(currentEntry);
        ADVLINE();
        ...
        while (t--)
        {
            ...
            ADVLINE();
            BalasanB b;
            TEXTB(b) = currentEntry;
            ADVLINE();
            AUTHORB(b) = currentEntry;
            ADVLINE();
            WAKTUB(b) = entryToDateTime(currentEntry);
            ADVLINE();
            IDB(b) = IDBalas;
            // printf("| ID = %lld\n", IDB(b));
...
void MuatDraf(char *Path, ListStatik *ListProfil)
{
    ...
    *folder = '\0';
    ...
    while (n--)
    {
        ...
        {
            Eltypes ns;
            TEXTS(ns) = currentEntry;
            ADVLINE();
            WAKTUS(ns) = entryToDateTime(currentEntry);
            ADVLINE();
...
void MuatUtas(char *Path, ListDinT *KicauGlobal, ListDin *UtasGlobal)
{
    ...
    *folder = '\0';
    ...
    while (n--)
    {
        ...
        {
            UTAS u;
            Entry textu = currentEntry;
            ADVLINE();
            Entry authoru = currentEntry;
            ADVLINE();
            DATETIME d = entryToDateTime(currentEntry);
            ADVLINE();
